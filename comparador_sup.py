...
def Comparar(dir_simulacion, carpetas_escenarios, intervalos_escenarios, cripto, bots):
  ...
  for i, carpeta_escenario in enumerate(carpetas_escenarios):
    ...
    for bot in bots:
      ...
      fecha_inicial_str = intervalos_escenarios[i][0]
      fecha_final_str = intervalos_escenarios[i][-1]
      
      dict_resultados["Desde"] = fecha_inicial_str
      dict_resultados["Hasta"] = fecha_final_str
      ...
      fecha_inicio = datetime.fromisoformat(fecha_inicial_str)
      
      index = df_historia[df_historia["date"] == str(fecha_final.date())].index
      ...
      proyeccion_anual = rendimiento_periodo * (365/(fecha_final - fecha_inicio).days)      
