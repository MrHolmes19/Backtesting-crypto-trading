...
for i, p in enumerate(periodos):
  df[f'volume_{p[0]}'] = round(volume.rolling(window=p[1]).sum(),1)
  
#print(df)
print("Guardando CSV de volumen de transacciones...")
df.to_csv(f"volumenes de transacciones.csv", encoding='utf-8', index=True)
...
for i, p in enumerate(periodos):
  vela = p[0]    
  media = df2[f'volume_{p[0]}'].mean().round(4)
  mediana = df2[f'volume_{p[0]}'].median()
  max = df2[f'volume_{p[0]}'].max()
  min = df2[f'volume_{p[0]}'].min()
  moda = df2[f'volume_{p[0]}'].mode()[0]
  desv_std = df2[f'volume_{p[0]}'].std().round(4)
...
writer = pd.ExcelWriter('volumenes de transaccion historicos.xlsx', engine = "xlsxwriter")
...
fmt_numerico = wb.add_format({'num_format': '0.0', 'align': 'center', 'text_wrap': True})

# Aplico formatos a las columnas
ws.set_column('A:A', 20, fmt_gral)
ws.set_column('B:I', 10, fmt_numerico)
...
try:
  os.mkdir(f"volumenes de transacciones historicos")
except:
  pass
os.chdir(f"volumenes de transacciones historicos")
...
for j, p in enumerate(df_est.index):
  ...
  lista_graficos = [
    partial(graficador_generico.lineal, 
            df2, df2.index, df2[f'volume_{vela}'], label, f"Serie temporal - {vela}",media, desv_std),    
    partial(graficador_generico.caja_y_bigotes,
            df2[f'volume_{vela}'], label, f"Caja y bigotes - {vela}"),
    partial(graficador_generico.histograma, 
            df2[f'volume_{vela}'], label, f"Histograma - {vela}", media, mediana, min, max, desv_std),
    partial(graficador_generico.distribucion_acumulada, 
            df2[f'volume_{vela}'], label, f"Distribucion acumulada - {vela}", media, min, max, desv_std)
  ]    
  
  graficador_generico.multiplot(2, 2, 
                                f"Volumenes de transacciones - Velas de {vela} - Histórico", 
                                lista_graficos, 
