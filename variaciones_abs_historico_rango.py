...
for i, p in enumerate(periodos):
  df[f'dif_{p[0]}'] = round(abs(high.rolling(window=p[1]).max()/low.rolling(window=p[1]).min() - 1),4)

#print(df)
print("Guardando CSV de variaciones...")
df.to_csv(f"variaciones_abs_historico_rango.csv", encoding='utf-8', index=True)
